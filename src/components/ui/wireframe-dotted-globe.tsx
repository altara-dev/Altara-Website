...
export default function RotatingEarth({ width = 800, height = 600, className = "" }: RotatingEarthProps) {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Draw ocean (globe background) with blue tint
      context.beginPath()
      context.arc(containerWidth / 2, containerHeight / 2, currentScale, 0, 2 * Math.PI)
      
      // Create radial gradient for globe background
      ...
      if (landFeatures) {
        // Draw graticule with blue color
        ...
        // Draw land outlines with light blue
        context.beginPath()
        ...
        // Draw halftone dots with light blue color
        allDots.forEach((dot) => {
    ...
    return (
      <div className={`flex items-center justify-center ${className}`} style={{ width, height }}>
        <div className="text-center text-muted-foreground">
          <p className="text-lg font-medium">Error loading Earth visualization</p>
  ...
  return (
    <div className={`relative ${className}`}>
      <canvas
        ref={canvasRef}
        className={`cursor-grab active:cursor-grabbing transition-opacity duration-500 ${isLoading ? 'opacity-0' : 'opacity-100'}`}
      />
      {isLoading && (
